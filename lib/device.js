...
function Device(adapter, connection, gpsServer) {
  ...
  this.run_other  = function(msgParts) {
    _this.emit('other', msgParts);
    _this.adapter.run_other(msgParts.cmd, msgParts);
  };

  this.set_fuel = function(val) {
    if (parseInt(val)==1){
      _this.adapter.open_fuel();
    }else{
      _this.adapter.cut_fuel();
    }
  };
