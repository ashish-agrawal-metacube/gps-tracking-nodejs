...
//    var crc = require('crc16-itu');
  ...
  this.receive_alarm = function (msg_parts) {
    ...
    var data = {
      ...
      'sat': parseInt(str.substr(13, 1),16),
    ...
    res = {
      ...
      speed: data.speed,
      sat: data.sat,
      course_lat: data.orientation["latitude"],
      course_lon: data.orientation["longitude"],
      course:  data.orientation["course"],
      mcc: data.lbs["mcc"],
      mnc: data.lbs["mnc"],
      lac: data.lbs["lac"],
      cid: data.lbs["cell_id"],
      ignition: !!data['power_status'],
      gps: !!data['gps_status'],
      alarm_status: data['alarm_status'],
      charge: !!data['charge_status'],
      acc: !!data['acc_status'],
      defence: !!data['defence_status'],
      power: data.power["level"],
      power_status: data.power["status"],
      gsm: data.gsm["level"],
      gsm_status: data.gsm["status"]
    };
    return res;
  };
  ...
  this.lbs_info = function (data) {
    ...
    var lac = parseInt(data.substr(6,4),16); // Location Area Code
    var cell_id = parseInt(data.substr(10,6),16); // Cell Tower ID
    return {
      mcc: mcc,
      mnc: mnc,
      lac: lac,
...
// console.log(new adapter().receive_alarm({data: '10061d0b212dc7022517c904b5dd3b0038e60902d4022bdf009cba50066402010006d2b5'}));

// console.log(new adapter().receive_alarm({data: '120405051704c702dfda550822a839001541090194005a78002fd14c030403022806d1150d'}));
